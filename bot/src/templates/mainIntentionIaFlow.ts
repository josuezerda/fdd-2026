...
const promptDetected = fs.readFileSync(
  path.join(__dirname, "..", "assets", "Prompt", "prompt_Detection.txt"),
  "utf8"
);
    ...
    afterEnd(flow) {
      return flow.addAction(async (_, { state, gotoFlow, flowDynamic }) => {
        try {
          ...
          switch (intention) {
            case "FAQ_FIESTA": return gotoFlow(faqFiestaFlow);
            case "MENU_OPCIONES": return gotoFlow(menuFlow);
            case "NO_DETECTED":
            default:
              await flowDynamic(
          ...
          console.log("Error en DetectIntention:", error);
        }
      });
    },
  });
